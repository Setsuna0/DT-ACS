...
for outStep=1:loopStep
    ...
    for innerStep=1:learningStep
        ...
        % difference = R + alpha * min Q(s`,a`) - Q(s, a)
        % temporal difference
        if nextState == 1  
            NactionIndex = 1;
        elseif nextState == M
            NactionIndex = actionNumber;
        else
            if rand(1,1)<epsilon    
                % epsilon-greedy policy, in fact it is to generate epsilon-greedy action at next state
                NactionIndex = ceil(rand(1,1) * (actionNumber-2))+1;
            else
                [minimalQvalue, NactionIndex] = min(Qfactor(currentState,:));
            end
            greedyPolicy(currentState) = actionSet(NactionIndex);  
            % generate the action of the next state which is epsilon greedy
        end
        if nextState == 17
            nextState = 16;
        end
        difference = costDiscouted + endSojournTime * Qfactor(nextState, NactionIndex) - Qfactor(currentState,actionIndex);
